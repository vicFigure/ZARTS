import torch
import torchvision
from operations_aa import apply_augment, Lighting
from torch.utils.data import SubsetRandomSampler, Sampler, Subset, ConcatDataset
from sklearn.model_selection import StratifiedShuffleSplit
from genotypes import sub_policies
...
class AugmentDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.search:
            ...
            return img, target
            ##############
            # I think the augmentation should be applied in the training procedure.
            ##############
#            img = self.pre_transforms(img)
...
#                    image = apply_augment(image, ops_name, magnitude[i].item())
...
#            return img, target
    ...
    def apply_transforms(self, imgs, sample_policy_idx):
      if self.search:
        policy = []
        applied_op_idx = []
        for k, idx in enumerate(self.weights_index):
          idx = idx.item()
          if idx in applied_op_idx: continue # If the sup-policy has been applied, then we donot apply it again
          applied_op_idx.append(idx)
          magnitude = self.magnitudes.clamp(0,1)[sample_policy_idx, k]
          sub_policy = self.ops_names[idx]
          probability_index = self.probabilities_index[sample_policy_idx, k]
          policy.append((sub_policy, probability_index, magnitude))
        images = []
...
#          img = self.pre_transforms(img)
          image = img
          for tmp in policy:
            sub_policy, probability_index, magnitude = tmp
            if probability_index.item() != 0.0:
              image = apply_augment(image, sub_policy, magnitude.item())
          image = self.after_transforms(image)
          images.append(image)
...
def get_dataloaders(dataset, batch, num_workers, dataroot, ops_names, magnitudes, cutout, cutout_length, split=0.5, split_idx=0, target_lb=-1, valid_aug=False):
...
#            Lighting(0.1, _IMAGENET_PCA['eigval'], _IMAGENET_PCA['eigvec']),
    ...
    if dataset == 'cifar10':
        ...
        total_trainset = ImageNet(root=os.path.join(dataroot, 'ILSVRC2012'), transform=None)
        ...
        total_trainset = ImageNet(root=os.path.join(dataroot, 'ILSVRC2012'), transform=None, download=False)
        # testset = ImageNet(root=os.path.join(dataroot, 'imagenet-pytorch'), split='val', transform=transform_test)
...
#        sss = StratifiedShuffleSplit(n_splits=1, test_size=len(filter_train_idx) - 6000, random_state=0)  # 4000 trainset
        sss = StratifiedShuffleSplit(n_splits=1, test_size=5000, random_state=0)  # 4000 trainset
    ...
    train_sampler = None
    if split > 0.0:
        sss = StratifiedShuffleSplit(n_splits=split_idx+1, test_size=split, random_state=0)
        sss = sss.split(list(range(len(total_trainset))), total_trainset.targets)
...
#        valid_sampler = SubsetSampler(valid_idx)
    ...
    valid_data = AugmentDataset(total_trainset, transform_train_pre, transform_train_after, transform_test, ops_names, valid_aug, magnitudes)
#    valid_data = AugmentDataset(total_trainset, transform_train_pre, transform_train_after, transform_test, ops_names, True, magnitudes)
    ...
    #     drop_last=False
    # )
    print("Num. of train set and valid set: %d / %d"%(len(train_data), len(valid_data)))
